...
<div class="my-8 flex flex-col items-center gap-20 min-h-full">
    ...
    <div class="flex flex-col w-11/12 items-center gap-20">
        {#each lim ? projects.slice(0, 2) : projects as project, index}
            <ProjectCard {project} {index}/>
        {/each}
    </div>
    {#if projects.length > 2}
        <button on:click={() => (lim = !lim)} class="w-32 h-11 border rounded-lg border-purple text-purple text-base">
